...
struct redblacktree<T>::Node {
	...
	Node(	T v = T(), bool RB = true,
		...
		: key(v), RBc(RB), ftr(f), lc(l), rc(r), s(ss) 
	{
		//value = log_malloc(128, &value);
		value = yesmalloc(128);
...
void redblacktree<T>::connect34(	Node* nroot,	Node* nlc,		Node* nrc,
	...
	nlc->lc = ntree1;if(ntree1 != NULL) ntree1->ftr = nlc;
	nlc->rc = ntree2;if(ntree2 != NULL) ntree2->ftr = nlc;
	nrc->lc = ntree3;if(ntree3 != NULL) ntree3->ftr = nrc;
	nrc->rc = ntree4;if(ntree4 != NULL) ntree4->ftr = nrc;
...
void redblacktree<T>::init(T v) {
    ...
    /*
	_root->key = v;
    _root->value = log_malloc(128, &(_root->value));
    memset(_root->value, -1, sizeof(_root->value));
    _root->RBc = false;
    _root->ftr = NULL;
    _root->lc = NULL;
    _root->rc = NULL;
...
int redblacktree<T>::find_rank(T v, Node* ptn) {
	if(!ptn) return 1;
	else if(ptn->key >= v) return find_rank(v, ptn->lc);
...
redblacktree<T>::iterator redblacktree<T>::insert(T v) {
    ...
    /*
	ptn->key = v;
    ptn->value = log_malloc(128, &(ptn->value));
    memset(ptn->value, -1, sizeof(ptn->value));
    ptn->RBc = true;
    ptn->ftr = _hot;
    ptn->lc = NULL;
    ptn->rc = NULL;
